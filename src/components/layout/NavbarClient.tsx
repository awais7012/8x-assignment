...
/*
 * The auth controls have to render correctly in three states: signed out,
 * signed in, and Clerk-not-configured (the landing page must work with no
 * keys). `Show` only exists inside ClerkProvider, so it is only mounted when
 * the server told us Clerk is enabled; otherwise the static links render.
 */
function AuthLinks({
  mobile,
  onNavigate,
        ...
        <div className="ml-auto hidden items-center gap-2 md:flex">
          {clerkEnabled ? (
            <>
              ...
              <ClerkLoaded>
                <Show when="signed-out" fallback={<UserButton showName />}>
                  <AuthLinks />
                </Show>
              </ClerkLoaded>
            </>
